...
if (typeof ChromeUtils === "object" && typeof ChromeUtils.import === "function") {
    ...
    var { MdnUtils } = ChromeUtils.import("chrome://unifiedsearch/content/MdnUtils.js");
} else {
    try {
      var { AppConstants } = Components.utils.import("resource://gre/modules/AppConstants.jsm", null); // Gecko 45+, STEEL (Application) is removed in Gecko 57
    } catch(ex) { };
    var { MdnUtils } = Components.utils.import("chrome://unifiedsearch/content/MdnUtils.js", null);
}
...
var unifiedsearch = {
    ...
    /* modified version of filterFromSearchBox (this doesn't have 'autoShowFilterbar') */
    ...
    loadFilterAutoComplete: function(gsbox, qfbox) {
        //TODO:WONTFIX: Not fully implemented, not used, and maybe unnecesary
        gsbox.controller.input = gsbox;
    ...
    switchUnifiedSearchAutoComplete: function(usbox) {
        if (!usbox) {
            usbox = this.usbox; // crazy? Not, I must review Why the param.
        }
    ...
    */
    clearAllFilteringOptions: function() {
        ...
        if (this.gsbox && this.options.enableFilteringInSearchBox) {
            this.gsbox.value = '';
        }
    ...
    */
    ...
    /* Adds shortcuts to the filter options in the standard Quick Filter Box,
       to behave like the Unified Search Box (that includes shortcuts by default, but standard Thunderbird doesn't) */
    configureShortcutsQFBox: function() {
        ...
        */
        document.getElementById('qfb-qs-sender').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-qs-sender').getAttribute("accesskey"));
        document.getElementById('qfb-qs-recipients').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-qs-recipients').getAttribute("accesskey"));
        document.getElementById('qfb-qs-subject').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-qs-subject').getAttribute("accesskey"));
        document.getElementById('qfb-qs-body').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-qs-body').getAttribute("accesskey"));

        document.getElementById('qfb-unread').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-unread').getAttribute("accesskey"));
        document.getElementById('qfb-starred').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-starred').getAttribute("accesskey"));
        document.getElementById('qfb-inaddrbook').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-inaddrbook').getAttribute("accesskey"));
        document.getElementById('qfb-tags').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-tags').getAttribute("accesskey"));
        document.getElementById('qfb-attachment').setAttribute('accesskey', document.getElementById('unifiedsearch-widget-menu-attachment').getAttribute("accesskey"));
    },
